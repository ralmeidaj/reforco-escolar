          ...
          : sessions.length === 0
            ...
            : sessions.map((sess) => (
                <Card key={sess.id}>
                  <View style={row.row}>
                    <View style={{ flex: 1 }}>
                      <Text style={s2.title}>{sess.subject?.name ?? 'Aula'}</Text>
                      <Text style={s2.sub}>Prof. {sess.teacher?.name ?? '—'}</Text>
                      <Text style={s2.sub}>{new Date(sess.scheduledAt).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}</Text>
                    </View>
                    <Badge label={sess.status} variant={STATUS_VARIANT[sess.status] ?? 'muted'} />
                  </View>
                </Card>
              ))
        }
