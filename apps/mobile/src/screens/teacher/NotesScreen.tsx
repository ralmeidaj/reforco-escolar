...
export function NotesScreen() {
  ...
  async function openSession(sess: Session) {
    ...
    try {
      ...
      if (res.data.length > 0) {
        ...
        setExistingNoteId(res.data[0].id);
      }
    } catch {}
  }
  ...
  async function save() {
    ...
    try {
      let res;
      if (existingNoteId) {
        res = await api.patch(`/session-notes/${existingNoteId}`, { content: content.trim() });
      } else {
        res = await api.post('/session-notes', { sessionId: selected.id, content: content.trim() });
      }
      setNotes([res.data]);
      ...
      <SafeAreaView style={s.safe}>
        ...
        <ScrollView contentContainerStyle={s.content}>
          ...
          <View style={s.inputBox}>
            <Text style={s.label}>{existingNoteId ? 'Editar nota de aula' : 'Nova nota de aula'}</Text>
            <TextInput
              style={s.textarea}
              value={content}
