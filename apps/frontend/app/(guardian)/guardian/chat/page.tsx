...
export default function GuardianChatPage() {
  ...
  useEffect(() => {
    ...
    api.get<Message[]>(`/messages/conversation/${selectedContact}`).then(({ data }) => {
      setMessages(data);
      bottomRef.current?.scrollIntoView({ behavior: 'smooth' });
    });
    api.post(`/messages/read/${selectedContact}`).catch(() => {});
  }, [selectedContact]);
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setSending(true);
    try {
      const { data } = await api.post<Message>('/messages', { toId: selectedContact, content: text });
      ...
      <div className="w-64 shrink-0 rounded-2xl bg-white shadow-sm overflow-y-auto">
        ...
        {contacts.length === 0 ? (
          ...
          contacts.map((c) => (
            <button
              key={c.userId}
              onClick={() => setSelectedContact(c.userId)}
              className={`w-full flex items-center justify-between px-4 py-3 text-left hover:bg-gray-50 transition-colors ${
                selectedContact === c.userId ? 'bg-brand-50' : ''
              }`}
            >
              <span className="text-sm text-gray-900 truncate">{c.name ?? c.userId.slice(0, 8)}</span>
              {c.unread > 0 && (
