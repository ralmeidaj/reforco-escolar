...
function AcceptInviteForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
      ...
      >
        {loading ? <span className="flex items-center justify-center gap-2"><Spinner size="sm" className="text-white" />Criando conta...</span> : 'Criar minha conta'}
      </button>
    </form>
  );
}
...
export default function AcceptInvitePage() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-6 py-12">
      <Suspense fallback={<Spinner size="lg" className="text-brand-600" />}>
        <AcceptInviteForm />
      </Suspense>
    </div>
  );
