...
- **Adicionar aluno manualmente pelo admin:** `POST /rooms/:id/admin-checkin` (`tenant_admin`, autenticado) — botão "+ Adicionar aluno" em cada sala na tela `/admin/rooms`, com busca por nome entre todos os alunos do tenant. Reaproveita o mesmo `RoomsService.checkin` usado pelo aluno (self-checkin) e pelo kiosk — respeita capacidade da sala, tira automaticamente de outra sala se já estiver em uma, e tenta vincular professor/criar sessão do mesmo jeito. Complementa o botão "Encerrar" que já existia em "Alunos no reforço agora", que remove — assim admin adiciona e remove pela mesma tela, sem precisar do kiosk
